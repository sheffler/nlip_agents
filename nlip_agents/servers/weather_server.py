...
class WeatherManager(SessionManager):
...
server.SESSION_MANAGER_CLASS = WeatherManager
server.SESSION_COOKIE_NAME = "WeatherCookie"

app = server.app

if __name__ == "__main__":
    # uvicorn.run(app, host="0.0.0.0", port=8022, log_level="info")
    uvicorn.run("nlip_agents.servers.weather_server:app", host="0.0.0.0", port=8022, log_level="info", reload=True)
